...
class ApplicationMethodService:
    ...
    async def _analyze_fertilizer_specification(self, fertilizer_spec: FertilizerSpecification) -> Dict[str, Any]:
        ...
        # Fetch detailed fertilizer type information
        detailed_fertilizer_type = self.fertilizer_type_service.get_fertilizer_type_by_id(fertilizer_spec.fertilizer_type)
        if detailed_fertilizer_type:
            analysis["detailed_type_info"] = detailed_fertilizer_type.dict()
            analysis["application_method_compatibility_from_type"] = detailed_fertilizer_type.application_method_compatibility
            analysis["environmental_impact_profile_from_type"] = detailed_fertilizer_type.environmental_impact_profile
            analysis["release_characteristics_from_type"] = detailed_fertilizer_type.release_characteristics
        else:
            logger.warning(f"Detailed info for fertilizer type {fertilizer_spec.fertilizer_type} not found.")
    ...
    async def _score_application_methods(
        ...
        for method_type, method_data in self.method_database.items():
            ...
            fertilizer_score = self._calculate_fertilizer_compatibility_score(
                method_type, method_data["fertilizer_forms"], fertilizer_analysis["form"],
                fertilizer_analysis.get("application_method_compatibility_from_type", [])
            )
            score += fertilizer_score * 0.20
            
            # Environmental impact (10% weight)
            environmental_score = self._calculate_environmental_score(
                method_data["environmental_impact"],
                fertilizer_analysis.get("environmental_impact_profile_from_type", [])
            )
    ...
    def _calculate_fertilizer_compatibility_score(self, method_type: ApplicationMethodType, method_forms: List[FertilizerForm], fertilizer_form: FertilizerForm, detailed_method_compatibility: List[ApplicationMethodCompatibility]) -> float:
        """Calculate fertilizer form compatibility score, considering detailed fertilizer type compatibility."""
        form_score = 1.0 if fertilizer_form in method_forms else 0.0

        # If the specific fertilizer type has explicit compatibility info, use it
        if detailed_method_compatibility:
            if method_type.value in [comp.value for comp in detailed_method_compatibility]:
                form_score = max(form_score, 0.9) # Boost if explicitly compatible
            else:
                form_score = min(form_score, 0.1) # Penalize if explicitly incompatible (and not already 0)

        return form_score
    
    def _calculate_environmental_score(self, method_environmental_impact: str, fertilizer_environmental_profile: List[EnvironmentalImpactProfile]) -> float:
        """Calculate environmental impact score (higher is better), considering fertilizer type profile."""
        impact_scores = {
            "very_low": 1.0,
            "low": 0.8,
            "moderate": 0.6,
        ...
        base_score = impact_scores.get(method_environmental_impact, 0.6)

        # Adjust based on fertilizer's specific environmental profile
        for profile in fertilizer_environmental_profile:
            if profile == EnvironmentalImpactProfile.LOW_LEACHING:
                base_score = min(1.0, base_score + 0.1)
            elif profile == EnvironmentalImpactProfile.HIGH_LEACHING:
                base_score = max(0.0, base_score - 0.2)
            elif profile == EnvironmentalImpactProfile.LOW_VOLATILIZATION:
                base_score = min(1.0, base_score + 0.1)
            elif profile == EnvironmentalImpactProfile.HIGH_VOLATILIZATION:
                base_score = max(0.0, base_score - 0.2)
            elif profile == EnvironmentalImpactProfile.ORGANIC_SOURCE:
                base_score = min(1.0, base_score + 0.1)
