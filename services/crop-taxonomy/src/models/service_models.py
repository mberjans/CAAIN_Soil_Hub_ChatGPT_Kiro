...
# ============================================================================
# ATTRIBUTE TAGGING MODELS
# ============================================================================

class TagCategory(str, Enum):
    """Categories for crop attribute tags."""
    TAXONOMY = "taxonomy"
    AGRONOMIC = "agronomic"
...
class TagType(str, Enum):
    """Types describing how a tag was created."""
    AUTO = "auto"
    MANUAL = "manual"
    VALIDATED = "validated"


class TagValidationStatus(str, Enum):
    """Validation status for crop attribute tags."""
    PENDING = "pending"
    VALIDATED = "validated"
    REJECTED = "rejected"


class CropAttributeTagPayload(BaseModel):
    """Payload representing a crop attribute tag."""

    tag_id: Optional[UUID] = Field(None, description="Unique identifier for the tag")
    crop_id: UUID = Field(..., description="Crop identifier associated with the tag")
    tag_name: str = Field(..., description="Display value for the tag")
    normalized_tag: Optional[str] = Field(None, description="Normalized version of the tag for matching")
    tag_category: TagCategory = Field(..., description="Category assigned to the tag")
    tag_type: TagType = Field(default=TagType.AUTO, description="How the tag was created")
    validation_status: TagValidationStatus = Field(default=TagValidationStatus.PENDING, description="Validation status")
    confidence_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Confidence in tag accuracy")
    source: Optional[str] = Field(None, description="Source used to generate the tag")
    usage_count: int = Field(default=0, ge=0, description="Number of times the tag was used")
    last_used_at: Optional[datetime] = Field(None, description="Timestamp for when tag was last used")
    last_generated_at: Optional[datetime] = Field(None, description="Timestamp for when tag was generated")
    parent_tag_id: Optional[UUID] = Field(None, description="Optional parent tag for hierarchy")
    validation_notes: Optional[str] = Field(None, description="Additional notes about validation decisions")
    ...
    @validator('normalized_tag')
    def validate_normalized_tag(cls, value: Optional[str]) -> Optional[str]:
        """Trim normalized tag if provided."""
        if value is None:
            return None
        trimmed = value.strip()
        if trimmed:
...
class AutoTagGenerationRequest(BaseModel):
    """Request payload for auto-generating crop attribute tags."""

    crop_id: UUID = Field(..., description="Crop to generate tags for")
    include_existing_tags: bool = Field(default=True, description="Include existing manual tags in response")
    include_related_crops: bool = Field(default=True, description="Leverage related crops during generation")
    include_manual_tags: bool = Field(default=True, description="Return manual tags alongside autogenerated tags")
    maximum_tags: int = Field(default=20, ge=1, le=100, description="Maximum number of tags to return")
    forced_categories: Optional[List[TagCategory]] = Field(None, description="Limit generation to selected categories")

    @validator('forced_categories')
    def validate_forced_categories(cls, categories: Optional[List[TagCategory]]) -> Optional[List[TagCategory]]:
        """Ensure forced categories list is not empty if provided."""
        if categories is None:
            return None
        if len(categories) == 0:
            raise ValueError("forced_categories cannot be empty")
        return categories


class AutoTagGenerationResponse(BaseModel):
    """Response payload for auto-generated crop attribute tags."""

    crop_id: UUID = Field(..., description="Crop identifier")
    generated_tags: List[CropAttributeTagPayload] = Field(default_factory=list, description="Tags generated during the request")
    existing_tags: List[CropAttributeTagPayload] = Field(default_factory=list, description="Previously stored tags")
    confidence_summary: Dict[str, float] = Field(default_factory=dict, description="Confidence summary by category")
    generation_notes: List[str] = Field(default_factory=list, description="Notes about tag generation decisions")


class TagManagementAction(str, Enum):
    """Supported management actions for crop attribute tags."""
    ADD = "add"
    UPDATE = "update"
    REMOVE = "remove"
    VALIDATE = "validate"
...
class TagManagementInstruction(BaseModel):
    """Instruction describing a single tag management operation."""

    action: TagManagementAction = Field(..., description="Action to perform")
    tag: Optional[CropAttributeTagPayload] = Field(None, description="Tag payload for add or update operations")
    tag_id: Optional[UUID] = Field(None, description="Tag identifier for update or removal")
    validation_status: Optional[TagValidationStatus] = Field(None, description="Validation status override")
    notes: Optional[str] = Field(None, description="Operator notes for auditing")

    @validator('tag')
    def validate_tag_payload(cls, value: Optional[CropAttributeTagPayload], values: Dict[str, Any]) -> Optional[CropAttributeTagPayload]:
        """Ensure tag payload is provided when required."""
        action_value = values.get('action')
        if action_value in (TagManagementAction.ADD, TagManagementAction.UPDATE):
            if value is None:
                raise ValueError("Tag payload is required for add and update actions")
        return value

    @validator('tag_id')
    def validate_tag_identifier(cls, value: Optional[UUID], values: Dict[str, Any]) -> Optional[UUID]:
        """Ensure tag identifier is provided for update, remove, or validate actions."""
        action_value = values.get('action')
        if action_value in (TagManagementAction.UPDATE, TagManagementAction.REMOVE, TagManagementAction.VALIDATE):
            if value is None:
                raise ValueError("tag_id is required for update, remove, or validate actions")
        return value


class TagManagementRequest(BaseModel):
    """Request payload for managing crop attribute tags."""

    crop_id: UUID = Field(..., description="Crop identifier for tag management")
    instructions: List[TagManagementInstruction] = Field(default_factory=list, description="Management instructions to apply")
    apply_auto_validation: bool = Field(default=True, description="Automatically evaluate validation rules")

    @validator('instructions')
    def validate_instructions(cls, instruction_list: List[TagManagementInstruction]) -> List[TagManagementInstruction]:
        """Ensure there is at least one instruction to process."""
...
class TagManagementResponse(BaseModel):
    """Response payload for crop attribute tag management."""

    crop_id: UUID = Field(..., description="Crop identifier")
    applied_tags: List[CropAttributeTagPayload] = Field(default_factory=list, description="Tags applied or updated")
    removed_tag_ids: List[UUID] = Field(default_factory=list, description="Identifiers of removed tags")
