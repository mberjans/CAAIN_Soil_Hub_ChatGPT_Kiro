"""
Integration configuration for fertilizer-strategy service communication.

This module provides configuration settings for HTTP-based communication
between the fertilizer-strategy service and other CAAIN Soil Hub services,
particularly the fertilizer-timing service.
"""
...
def load_integration_config() -> IntegrationConfig:
    ...
    # Configure fertilizer-timing service
    fertilizer_timing_config = ServiceConfig(
        name="fertilizer-timing",
        base_url=os.getenv(
            "FERTILIZER_TIMING_URL",
            "http://localhost:8010"
        ),
        timeout=int(os.getenv("FERTILIZER_TIMING_TIMEOUT", "30")),
        max_retries=int(os.getenv("FERTILIZER_TIMING_MAX_RETRIES", "3")),
        retry_delay=float(os.getenv("FERTILIZER_TIMING_RETRY_DELAY", "1.0")),
        circuit_breaker_threshold=int(
            os.getenv("FERTILIZER_TIMING_CIRCUIT_BREAKER_THRESHOLD", "5")
        ),
        circuit_breaker_timeout=int(
            os.getenv("FERTILIZER_TIMING_CIRCUIT_BREAKER_TIMEOUT", "60")
        ),
        health_check_path=os.getenv("FERTILIZER_TIMING_HEALTH_PATH", "/health")
    )
    ...
    return IntegrationConfig(
        ...
        services={
            "fertilizer-timing": fertilizer_timing_config,
